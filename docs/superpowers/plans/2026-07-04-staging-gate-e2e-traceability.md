...
- Modify: `mcp_server.py` ~:1538 (found in Task 5 quality review): the reranker health check posts to a hardcoded `https://api.deepinfra.com/...`, ignoring `reranker_cfg.get("base_url")` it already holds — under staging config it would ping production DeepInfra with a real key on every health call. Route it through the config base_url (impact analysis first; add a unit test asserting the health check honors the override).
