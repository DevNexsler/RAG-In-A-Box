...
def test_embed_timeout_during_search_degrades_gracefully():
    ...
    When embed_query raises TimeoutError, hybrid_search should degrade to
    keyword-only search for pre-indexed documents instead of crashing.
    """
...
def test_embed_connection_refused_during_search():
    """AC-PROV-1: Embed provider connection refused -> keyword-only degradation.

    When embed_query raises ConnectionError, pre-indexed docs should still be
